...
if (menuList !== null) {
    menuList.forEach(item => {
        ...
        item.addEventListener('mouseleave', function() {
            setTimeout(resetAllMenuLists, 300)
        });
    }
    )
...
const tabs = () => {
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', () => {
            removeAllClasses(tabs, 'tabs__header-item--active');
            removeAllClasses(tabsContent, 'tabs__item--active');

            tab.classList.add('tabs__header-item--active');
            document.getElementById(tab.getAttribute('data-tab')).classList.add('tabs__item--active');
        }
        )
    }
    )
